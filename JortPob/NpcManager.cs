...
namespace JortPob
{
    public class NpcManager
    {
        ...
        /* For area bosses generated by dummy placements */
        public (int npc, int think) GetParams(ItemManager itemManager, BaseScript script, DummyContent content, Override.BossRemap bossRemap)
        {
            int npcRow = GetNpcParam(itemManager, script, content, bossRemap);
            int thinkRow = GetThinkParam(itemManager, script, content, bossRemap);

            return (npcRow, thinkRow);
        }

        private int GetNpcParam(ItemManager itemManager, BaseScript script, DummyContent content, Override.BossRemap bossRemap)
        {
            int level = Override.GetDifficultyLevel(content.cell);
            int id = nextNpcParamId += 10;
            paramanager.GenerateNpcParam(itemManager, script, content, level, id, bossRemap);
            return id;
        }

        private int GetThinkParam(ItemManager itemManager, BaseScript script, DummyContent content, Override.BossRemap bossRemap)
        {
            int id = nextNpcThinkParamId += 10;
            paramanager.GenerateThinkParam(itemManager, script, content, id, bossRemap);
            return id;
        }
